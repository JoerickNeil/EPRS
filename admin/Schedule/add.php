<?php 
	...
	while ($row = mysqli_fetch_array($result_record))
	{
		...
		$email = $row['email'];
		$Name = $row['Name'];
		$list_record .= "<option value='$RecordID'>Remarks: $Remarks || Name: $Name || Email: $email</option>";
		
		if(isset($_POST['recordID'])) {
			if($_POST['recordID'] == $RecordID) {
				$sendEmail = $email;
				$sendName = $Name;
			}
		}
	...
	if (isset($_POST['add']))
	{
		...
		if (mysqli_num_rows($result_check) == 0)
		{
								...
								else
								{
									...
									$message .= "";
									$message .= "<p>$Remarked</p>";
									$message .= "<p>Thank you.</p>";
								
									sendEmail($email, "Work Schedule", $message);
									echo $message;
...
<div class="form-group">
  ...
  <input class="form-control" name="dateadded" id="datepicker" type="date" min="<?php echo date('Y-m-d', strtotime("+ 1 day"));?>" value="This is focused...">
